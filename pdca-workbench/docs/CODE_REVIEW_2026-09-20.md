...
## 八、本轮拍板决策的落地情况（2026-09-20 补充）

| # | 决策 | 落地 | 证据 |
|---|---|---|---|
| 1 | 部门红黑榜全员可见（合并群 + C转B 群都发） | 已落地，文案显式标注「部门口径·全员可见」 | `app/duzhan.py` `_board_text(..., compact)`；`tests/test_duzhan.py` |
| 2 | 统一改叫「已录单 / 开单额」（去掉括号解释） | 已落地：三追标题、档位行、三口径行、日报、证据 HTML、README 全量改口径 | `app/duzhan.py`；`app/daily_digest.py:99,106,174,246`；`app/evidence_report.py` |
| 3 | 月度目标每月由用户更新（单一来源） | 已落地：文件优先 → 组目标摊人 → 硬编码兜底，兜底即告警（每月一次）；新增维护脚本 + 合计一致性校验 | `app/duzhan_ledger.py` `resolve_month_target`/`month_target_scope`/`warn_target_fallback`；`scripts/set_monthly_targets.py` |
| 4 | TLS 自己处理（去掉 `verify=False`） | 已落地：实测网关证书由公共 CA 签发，默认标准校验；内网 CA 走 `PDCA_QWEN_CA_BUNDLE` | `app/mto_ocr.py` `_tls_verify`；`tests/test_review_fixes_2026_09_20.py::TlsVerifyTests` |
| 5 | 采集并发 | 已落地：4 个数据源 / 各群历史 / 每人 3 项 MCP / 每人 MTO OCR 全部并发，单项失败隔离 | `app/duzhan_ledger.py` `_parallel_map`、`_mcp_bundle`；实测 2026-09-19：并发 20.4s vs 串行约 36.5s（-44%） |
| 6 | @回复幂等键 + 游标原子写 | 已落地：`duzhan-at-<message_id>` 幂等键；游标 tmp + `os.replace` 原子落盘，逐条回复各存一次 | `app/duzhan.py` `_save_cursor` |

### 仍未做（已记录，需排期）
- P1-2 `.gitignore` 与已入库生成物清理（涉及 66 个文件，需单独一次提交）。
- P2-4 前端内联 JS 的 CI 语法校验。
- P2-6 外部契约探针（每日只读干跑 + 告警）。
- 证据 HTML 附件 1MB × 3 收件人的 IM 体积优化；`test_duzhan` 用例会走真实 vertu-cli 采集（跑一轮要十几分钟），应改为 mock。
