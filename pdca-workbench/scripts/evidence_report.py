# -*- coding: utf-8 -*-
"""督战证据日报：把每人当天被系统读到的全部证据导成单文件 HTML，供人工抽检。
...
def main() -> int:
    ...
    ledger = collect(args.day, limit)
    html_text = build_html(args.day, ledger)
    out = Path(args.out) if args.out else Path.home() / "Desktop" / f"督战证据_{args.day}.html"
    ...
    compact = {
        ...
        "elapsed_seconds": ledger.get("_elapsed"),
        "people": [
            {
                "display": row.get("display"),
                "mtd_wan": row.get("perf_arrived_wan"),
                "rolling_target_wan": row.get("rolling_target_wan"),
                "slip": len(row.get("perf_slip") or []),
                "intent": len(row.get("perf_intent") or []),
                "mto": row.get("mto_count"),
                "wa_reached": row.get("wa_reached"),
                "hours_minutes": row.get("hours_minutes"),
                "daily_report": bool(row.get("daily_report")),
                "raw_messages": len((ledger.get("_raw_by_owner") or {}).get(str(row.get("display"))) or []),
            }
            for row in (ledger.get("people") or [])
        ],
    }
    sidecar.write_text(json.dumps(compact, ensure_ascii=False, indent=1), encoding="utf-8")
    print(f"HTML: {out} ({out.stat().st_size / 1024:.0f} KB)")
    print(f"JSON: {sidecar}")
    print(f"人数 {len(compact['people'])}｜耗时 {ledger.get('_elapsed')} 秒")
    return 0
