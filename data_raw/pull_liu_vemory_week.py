...
END = today.isoformat()
OUT_JSON = Path(__file__).resolve().parent / f"liu_vemory_week_{week_start.isoformat()}.json"

# 刘春梅本人 + 海外中台下属 + 海外经销商销售（含于冰组）
TARGETS = [
    (12564, "刘春梅"),
    (13365, "付汪阳"),
    (12545, "冯磊-1"),
...
def main() -> int:
    ...
    summary = {
        "window": {"from": START, "to": END, "label": f"本周 {START} ~ {END}"},
        "viewer": "刘春梅",
